...
parser.add_argument('--epoch',    default=5000, type=int, help='max epoch')

parser.add_argument('--scale',      default=8, type=int, help='scale factor')
parser.add_argument('--batch_size', default=40, type=int, help='batch_size')
parser.add_argument('--num_workers',default=4, type=int, help='num_workers')
parser.add_argument('--lr',         default='0.001', type=float, help='learning rate')
parser.add_argument('--step_size',  default=10000, type=int, help='step_size decrease lr')
...
parser.add_argument('--data_aug',   default=True, type=bool, help='data_augmentation')
...
if opt.model == 'FDKN':
    ...
    net = RRDB_backbone(up_scale=opt.scale, RRDB_channel=64, conv_kernel_size=3, beta=opt.beta, kpn_kernel=opt.k, residual=opt.residual).cuda()
    if opt.load:
...
if opt.data_aug:
    ...
    Train_dataset = NYU_v2_full_lr(depths=depths[:,:,ID_train],images=images[:,:,:,ID_train], scale=opt.scale, transform=transforms.ToTensor())
...
for epoch in range(max_epoch):
    ...
    for idx, data in enumerate(t):
        ...
        # nn.utils.clip_grad_value_(optimizer.,)
        # nn.utils.clip_grad_norm_(optimizer.parameters(), max_norm=10, norm_type=2)
        optimizer.step()
        scheduler.step()
