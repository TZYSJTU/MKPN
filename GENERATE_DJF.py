...
parser.add_argument('--weights',  default='./result//DJFR_8x/best', help='root to load model weights')
...
net = DJFR(residual=opt.residual).cuda()
if opt.load:
...
def validate_Sintel(net, dataloader):
    ...
    for idx, data in enumerate(t):    
        ...
        plt.axis('off')
        plt.imshow(out)
        plt.savefig("./Compare/DJFR/Sintel/{}_DJFR.jpg".format(idx), bbox_inches="tight",pad_inches=0.0)
        # plt.show()
        plt.close('all')
...
def validate_middlebury(net, dataloader):
    ...
    for idx, data in enumerate(t):    
        ...
        # plt.axis('off')
        # plt.imshow(out)
        # plt.savefig("./Compare/DJFR/Middlebury/{}_DJFR.jpg".format(idx), bbox_inches="tight",pad_inches=0.0)
        # # plt.show()
        # plt.close('all')
...
def validate_Lu(net, dataloader):
    ...
    for idx, data in enumerate(t):    
        ...
        # plt.axis('off')
        # plt.imshow(out)
        # plt.savefig("./Compare/DJFR/Lu/{}_DJFR.jpg".format(idx), bbox_inches="tight",pad_inches=0.0)
        # # plt.show()
        # plt.close('all')
...
def validate_nyu(net, dataloader,):
    ...
    for idx, data in enumerate(t):
        ...
        plt.axis('off')
        plt.imshow(out)
        plt.savefig("./Compare/DJFR/NYU/{}_DJFR.jpg".format(idx), bbox_inches="tight",pad_inches=0.0)
        # plt.show()
        plt.close('all')
