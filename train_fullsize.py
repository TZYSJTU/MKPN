...
parser.add_argument('--channel',    default=57, type=int, help='num_channels')
...
logging.info(
    'scale:%d batch_size:%d lr:%s step_size:%d gamma:%f beta:%f channel:%d residual:%d data_aug:%d load:%d' \
    %(opt.scale, opt.batch_size, opt.lr, opt.step_size, opt.gamma, opt.beta, opt.channel, opt.residual, opt.data_aug, opt.load)
    )
