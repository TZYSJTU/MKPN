...
parser.add_argument('--weights',  default='result/KPN/best', help='root to load model weights')
...
parser.add_argument('--gamma',      default=1, type=float, help='gamma decrease lr')
parser.add_argument('--beta',       default=0.2, type=float, help='residual scaling parameter')
...
if opt.model == 'FDKN':
    ...
    net = KPN_baseline(up_scale=opt.scale, RRDB_channel=64, conv_kernel_size=3, beta=opt.beta, kpn_kernel=opt.k, residual=opt.residual).cuda()
    if opt.load:
