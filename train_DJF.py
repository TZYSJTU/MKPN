...
result_root = '%s/DJFR_%s-s_%s'%(opt.result, s, opt.scale)
...
if opt.data_aug:
    Train_dataset = NYU_v2_full_hr_aug(depths=depths[:,:,ID_train],images=images[:,:,:,ID_train], scale=opt.scale, transform=transforms.ToTensor())
else:
